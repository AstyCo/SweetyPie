...
void IntervalSlider::paintEvent(QPaintEvent *event)
{
    ...
    drawHandle( &painter, getBeginHandleRect(),
                (m_clippedHandle == BeginHandle || /*m_shiftModifier*/ _pressed ) );
    drawHandle( &painter, getEndHandleRect(),
                (m_clippedHandle == EndHandle || /*m_shiftModifier*/ _pressed ) );
}
...
{
    return getBeginHandleRect().contains(pos);
}

bool IntervalSlider::posOverEndHandle(const QPoint& pos) const
{
    return getEndHandleRect().contains(pos);
}

bool IntervalSlider::posOverInterval(const QPoint &pos) const
{
    return getIntervalRect().contains(pos);
}
...
QRect IntervalSlider::getIntervalRect() const
{
    int begin = valueToPoint(m_beginValue, BeginHandle),
        end   = valueToPoint(m_endValue, EndHandle);
    return QRect(begin, 0, end - begin, rect().height());
}

QRect IntervalSlider::getSliderRect() const
{
    return QRect( rect().x() + halfHandleSize(),
                  rect().y() + (m_offsetV>0?m_offsetV:0),
                  rect().width() - 2 * halfHandleSize(), m_sliderV );
}
