...
void GanttIntervalGraphicsObject::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if(!innerInfo() || !m_scene)
        return;
    QColor color = (m_scene->currentItem() == this)?(innerInfo()->getColor().darker(130)):(innerInfo()->getColor());
...
void GanttIntervalGraphicsObject::updateItemGeometry()
{
    ...
    qreal startPos = m_header->dtToX(innerInfo()->start()),
          itemWidth = m_header->dtToX(innerInfo()->finish()) - startPos;
